...
class cliente{
    ...
    private function validar_datos(){
        ...
        if( empty( trim($this->datos['apellido'])) ){
            $this->respuesta['msg'] = 'por favor ingrese el apellido del cliente';
        }
    ...
    private function almacenar_cliente(){
        if( $this->respuesta['msg']==='correcto' ){
            if( $this->datos['accion']==='nuevo' ){
                $this->db->consultas('
                    INSERT INTO clientes (nombre,apellido,direccion,telefono,dui) VALUES(
                        "'. $this->datos['nombre'] .'",
                        "'. $this->datos['apellido'] .'",
    ...
    public function buscarCliente($valor = ''){
        $this->db->consultas('
            select clientes.idCliente, clientes.nombre, clientes.apellido, clientes.direccion, clientes.telefono, clientes.dui
            from clientes
            where clientes.nombre like "%'. $valor .'%" or clientes.apellido like "%'. $valor .'%" or clientes.dui like "%'. $valor .'%"
        ');
