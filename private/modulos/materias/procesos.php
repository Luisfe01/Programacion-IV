...
class materia{
    ...
    private function almacenar_materia(){
        if( $this->respuesta['msg']==='correcto' ){
            if( $this->datos['accion']==='nuevo' ){
                $this->db->consultas('
                    INSERT INTO materias (codigo,nombre,numorden,prerrequisito,ciclo) VALUES(
                        "'. $this->datos['codigo'] .'",
                        "'. $this->datos['nombre'] .'",
                        "'. $this->datos['numorden'] .'",
                        "'. $this->datos['prerrequisito'] .'",
                        "'. $this->datos['ciclo'] .'"
                ...
                $this->db->consultas('
                    UPDATE materias SET
                        ...
                        numorden  = "'. $this->datos['numorden'] .'",
                        prerrequisito   = "'. $this->datos['prerrequisito'] .'",
    ...
    public function buscarMateria($valor = ''){
        $this->db->consultas('
            select materias.idMateria, materias.codigo, materias.nombre, materias.numorden, materias.prerrequisito, materias.ciclo
            from materias
            where materias.codigo like "%'. $valor .'%" or materias.nombre like "%'. $valor .'%" or materias.numorden like "%'. $valor .'%"
